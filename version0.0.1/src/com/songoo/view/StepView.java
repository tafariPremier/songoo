...
public class StepView extends LinearLayout implements IStepView
{
	...
	{
		control.childIndex = _controls.size();
		_controls.put(control.childIndex, control);
	}
	
	public void removeControl(Control control)
	{
		if (_controls.containsKey(control.childIndex))
		{
			_controls.remove(control.childIndex);
		}
	}
	
	public void removeControl(int key)
	{
		Iterator<Control> iter = _controls.values().iterator();
		while (iter.hasNext())
		{
			Control c = iter.next();
			if (c.childIndex == key) iter.remove();
		}
	}
	...
	private void Initialize(Context context)
	{
		...
		_wadcResource = BitmapLoader.getBitmap(R.drawable.barcamp000);
	...
	{
		Control[] arrayControls = new Control[_controls.size()];
		arrayControls = _controls.values().toArray(new Control[0]);
		
		for(int i = 0; i < arrayControls.length; i++)
		{
			Control c = arrayControls[i]; //_controls.get(i);
